var x = [0, 90, 356, 170, 242, 381, 900, 660, 539, 670, 438, 102, 44, 605, 996, 85, 449, 15, 800, 21, 385, 317, 555, 399, 475, 621, 109, 602, 217, 893, 581, 274, 407, 611, 886, 368, 197, 897, 341, 651, 767, 364, 752, 803, 518, 838, 369, 10, 409, 848, 583, 885, 402, 200, 929, 816, 454, 424, 587, 72, 488, 269, 238, 958, 772, 185, 959, 685, 966, 889, 403, 629, 126, 166, 368, 391, 492, 142, 710, 503, 850, 996, 363, 588, 633, 293, 936, 960, 268, 491, 514, 41, 833, 948, 582, 709, 239, 341, 182, 191, 48, 32, 908, 540, 144, 110, 632, 141, 729, 522, 859, 954, 184, 286, 586, 845, 779, 866, 573, 74, 924, 786, 911, 768, 143, 157, 676, 446, 70, 120, 518, 490, 783, 222, 389, 225, 34, 450, 910, 873, 725, 324, 311, 963, 177, 558, 876, 65, 903, 2, 271, 305, 234, 146, 826, 92, 80, 533, 96, 136, 122, 969, 314, 323, 722, 457, 379, 838, 909, 809, 626, 704, 767, 90, 683, 447, 281, 357, 438, 825, 366, 991, 914, 962, 794, 61, 414, 670, 60, 349, 874, 753, 232, 527, 3, 27, 721, 350, 374, 917, 218];
for (let i = 1; i <= 181; i++) {
    var types = ["Normal ❎❎", "Cepat ❎✅", "Turbo ✅❎"];
    var checks = ["✅", "❎"];
    var labels = ["Auto!", "Manual!"];
    
    var lastTime = localStorage.getItem("lastTime_8_"+i);
    ...
    if (!lastTime || (currentTime - lastTime) >= 14400000) {
        ...
        localStorage.setItem("xx_8_"+i, xx);

        var random_val_1 = [15, 25, 35, 45, 55];
        var random_1 = random_val_1[Math.floor(Math.random() * random_val_1.length)];
        localStorage.setItem("random_1_xx_8_"+xx+"_"+i, random_1);

        var random_val_2 = [10, 20, 30, 40, 50];
        var random_2 = random_val_2[Math.floor(Math.random() * random_val_2.length)];
        localStorage.setItem("random_2_xx_8_"+xx+"_"+i, random_2);

        var random_val_3 = [3, 5, 7, 9];
        var random_3 = random_val_3[Math.floor(Math.random() * random_val_3.length)];
        localStorage.setItem("random_3_xx_8_"+xx+"_"+i, random_3);

        var random_val_4 = [20, 30, 40, 50, 60, 70];
        var random_4 = random_val_4[Math.floor(Math.random() * random_val_4.length)];
        localStorage.setItem("random_4_xx_8_"+xx+"_"+i, random_4);


        var random_type = Math.floor(Math.random() * types.length);
        if (random_type == 0) {
            localStorage.setItem("tips_type_number_1_xx_8_"+i, types[0]+""+checks[Math.floor(Math.random() * checks.length)]);
            localStorage.setItem("tips_type_number_2_xx_8_"+i, types[1]+""+checks[Math.floor(Math.random() * checks.length)]);
            localStorage.setItem("tips_type_number_3_xx_8_"+i, types[2]+""+checks[Math.floor(Math.random() * checks.length)]);
        }else if (random_type == 1) {
            localStorage.setItem("tips_type_number_1_xx_8_"+i, types[1]+""+checks[Math.floor(Math.random() * checks.length)]);
            localStorage.setItem("tips_type_number_2_xx_8_"+i, types[0]+""+checks[Math.floor(Math.random() * checks.length)]);
            localStorage.setItem("tips_type_number_3_xx_8_"+i, types[2]+""+checks[Math.floor(Math.random() * checks.length)]);
        }else if (random_type == 2) {
            localStorage.setItem("tips_type_number_1_xx_8_"+i, types[2]+""+checks[Math.floor(Math.random() * checks.length)]);
            localStorage.setItem("tips_type_number_2_xx_8_"+i, types[1]+""+checks[Math.floor(Math.random() * checks.length)]);
            localStorage.setItem("tips_type_number_3_xx_8_"+i, types[0]+""+checks[Math.floor(Math.random() * checks.length)]);
        }

        localStorage.setItem("tips_label_number_1_xx_8_"+i, labels[Math.floor(Math.random() * labels.length)]);
        localStorage.setItem("tips_label_number_2_xx_8_"+i, labels[Math.floor(Math.random() * labels.length)]);
        localStorage.setItem("tips_label_number_3_xx_8_"+i, labels[Math.floor(Math.random() * labels.length)]);
    }else{
        var xx = localStorage.getItem("xx_8_"+i);

        var random_1 = localStorage.getItem("random_1_xx_8_"+xx+"_"+i);
        var random_2 = localStorage.getItem("random_2_xx_8_"+xx+"_"+i);
        var random_3 = localStorage.getItem("random_3_xx_8_"+xx+"_"+i);
        var random_4 = localStorage.getItem("random_4_xx_8_"+xx+"_"+i);
    }
    ...
    $("#"+asd).parent().parent().parent().attr("onclick", "show_popup('xx_8_"+i+"', '"+asd+"', '"+random_1+"', '"+random_2+"', '"+random_3+"', '"+random_4+"')");
}
